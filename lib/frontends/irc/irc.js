    ...
    switch (message.command) {
      ...
      case "NOTICE":
        ...
        if (self.opt.debug && to == self.nick)
          console.log('NOTICE from ' + (from ? from : '(server)') + ': ' + text);
      break;
      case "MODE":
        if (self.opt.debug)
...
Client.prototype.connect = function (retryCount) { // {{{
  ...
  self.conn = (self.opt.secure ? tls.connect(self.opt.port, self.opt.server) : 
               net.createConnection(self.opt.port, self.opt.server));
    ...
    if (typeof self.opt.secure !== 'object') {
      ...
      self.conn.on("secure", function(){
        if(!self.conn.verifyPeer()) {
          if (self.opt.debug) {
            console.log('Warning: failed to verify SSL peer certificate'+
                    ' -- aborting connection');
          }
          this.end();
        }
  ...
  self.conn.on("error", function(msg) {
    self.emit("error", msg);
  });
}; // }}}
...
Client.prototype.send = function(command) { // {{{
  ...
  if (this.opt.debug)
    console.log('SEND: ' + command + " " + args.join(" "));

...
function Server(options/*, fnHandleNormalHTTP, fnAcceptClient*/) {
  ...
  var connections = this.connections = new Storage();
	...
	this.shutdown = function() {
		server.close();
    connections.do(function(user) { user.disconnect(); });
	};
}
...
Server.prototype.broadcast = function(data, encoding, except) {
  if (typeof encoding !== 'string' && typeof encoding !== 'undefined') {
    except = encoding;
    encoding = undefined;
  }
  this.connections.do(function(key, user) {
    if (!except || (key !== except && user !== except))
      user.write(data, encoding);
  });
};

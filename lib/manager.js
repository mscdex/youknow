...
// =============================================================================
var Player = function(name, type, game) {
  this.name = name;
  this.type = type;
...
Game.prototype.addPlayer = function(name, type) {
  var ret;
  if (this.state !== 'pregame')
...
Manager.prototype.addGame = function(gameName, ownerName, ownerType) {
  ...
  this.games[gameName].on('end', function() {
...
Manager.prototype.addPlayer = function(gameName, playerName, type) {
  var ret;
  if (typeof this.games[gameName] === 'undefined')
    return 'A game with that name doesn\'t exist';
  ...
  return this.games[gameName].addPlayer(playerName, type);
};
...
Manager.prototype.findPlayer = function(playerName, type) {
  ...
  for (var i=0,players,len=games.length; i<len; i++) {
    ...
    for (var j=0,plen=players.length; j<plen; j++) {
      if (players[j].type === type && check(players[j])) {
        ret = players[j];
        found = true;
        break;
      }
...
Manager.prototype.getPlayerNames = function(gameName, type, except) {
  ...
  for (var i=0,len=players.length; i<len; i++)
    if (players[i].type === type && (!except || check(players[i])))

 ...
 *       - Actually take advantage of 'game' property of Player instead
 *         of regular for-loop searches for game associated with a player
 *       - How to handle a game owner quitting? Auto-assign ownership to the next
 *         player in the list or destroy the game (ejecting everyone)?
 */
...
var Game = function(name) {
  ...
  this.dir = -1; // -1 for "clockwise" (left), 1 for "counter-clockwise" (right)
...
Game.prototype.addPlayer = function(name, type, userData) {
  var ret;
  if (this.state !== 'pregame')
...
Manager.prototype.addGame = function(gameName, ownerName, ownerType, userData) {
  ...
  if (typeof this.games[gameName] !== 'undefined')
    ...
    return ret;
  var game = this.games[gameName] = new Game(gameName);
  ...
  game.addPlayer(ownerName, ownerType, userData);
...
Manager.prototype.addPlayer = function(gameName, playerName, type, userData) {
  var ret;
  if (typeof this.games[gameName] === 'undefined')
    return 'A game with that name doesn\'t exist';
  ...
  return this.games[gameName].addPlayer(playerName, type, userData);
};
